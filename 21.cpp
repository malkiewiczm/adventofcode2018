...
static void print_op(int inst, tuple &o)
{
	if (o[0] == SETI || o[0] == SETR)
		printf("%d : %s %d %d\n", inst, OP_STR[o[0]], o[1], o[3]);
	else
		printf("%d : %s %d %d %d\n", inst, OP_STR[o[0]], o[1], o[2], o[3]);
}
...
static inline void run(vec_t &op_list, int ip)
{
	...
	while (sto[ip] >= 0 && sto[ip] < (int)op_list.size()) {
		...
		if (sto[ip] == 29) {
			...
			return;
#else
			if (r0values.find(sto[3]) != r0values.end()) {
				trace("prev = " << prev);
				return;
			}
			r0values.insert(sto[3]);
			prev = sto[3];
#endif
		}
	}
